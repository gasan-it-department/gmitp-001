...
>>>>>>> harvey
    ...
    return (
        <Sheet>
            ...
            <SheetContent side="left" className="flex h-full w-80 flex-col justify-between bg-background text-foreground shadow-2xl">
                <SheetTitle className="sr-only">Navigation Menu</SheetTitle>

                <SheetHeader className="border-b border-border bg-muted/10 px-5 py-6">
                    <h2 className="text-left text-xl font-black tracking-widest text-primary uppercase">Menu</h2>
                </SheetHeader>
                ...
                <div className="flex flex-1 flex-col overflow-y-auto py-2">
                    ...
                    {auth?.user ? (
                        <div className="px-4 py-4">
                            <Link href={profile.show.url({ municipality: currentMunicipality.slug })}>
                                <div className="flex cursor-pointer items-center rounded-2xl border border-border bg-muted/20 p-4 transition-all hover:border-primary/50 hover:bg-primary/5 hover:shadow-sm">
                                    <Avatar className="h-16 w-16 flex-shrink-0 bg-muted">
                                        <AvatarImage
                                            src={profilePic}
                                            alt="User avatar"
                                        ...
                                        {/* The fallback ensures you always see something even if the DB link is broken */}
                                        <AvatarFallback className="rounded-full font-bold text-muted-foreground uppercase">
                            ...
                            <div className="mt-6 border-b border-border pb-6">
                                <span className="mb-3 block text-[10px] font-black tracking-widest text-muted-foreground/60 uppercase">
                                    My Activity
                                </span>
                                <SheetClose asChild>
                                    <a
                                        ...
                                        className={cn(
                                            'flex cursor-pointer items-center gap-4 rounded-xl p-3 transition-all duration-200',
                                ...
                                return (
                                    <SheetClose asChild key={item.title}>
                                        <a
                                            ...
                                            className={cn(
                                                'group flex cursor-pointer items-center gap-4 rounded-xl p-3 transition-all duration-200',
                                                isActive
                                                    ? 'bg-primary/10 font-black text-primary shadow-sm' // Active Style
                                                    : 'font-medium text-muted-foreground hover:bg-muted hover:text-foreground', // Default Style
                                            )}
                                        >
                                            ...
                                            {Icon ? (
                                                ...
                                                <HomeIcon size={26} className={isActive ? 'text-primary' : ''} />
                                            )}
