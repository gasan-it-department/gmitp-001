...
export default function LoginPage() {
    ...
    const { data, setData, post, processing, errors } = useForm({
        login_id: '', // This will hold either the email or mobile number
        password: '',
        remember: false,
    });
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        // Since this is Inertia, you post to your Laravel web login route
        post('/login');
    };

    const handleGoogleLogin = () => {
        // Here is where you will trigger your React Google Auth flow
        // and send the token to our new /api/auth/google endpoint!
            ...
            <div className="flex w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm md:flex-row">
                ...
                <div className="flex w-full flex-col justify-center p-8 sm:p-12 md:w-1/2">
                    <div className="mb-8 text-center">
                        ...
                        <p className="mt-2 text-sm text-gray-500">Login to your Gasan LGU account</p>
                    </div>

                    <form onSubmit={submit} className="space-y-5">
                        {/* Email or Mobile Input */}
                        <div>
                            <label htmlFor="login_id" className="mb-1 block text-sm font-medium text-gray-700">
                            ...
                            <input
                                ...
                                required
                            />
                            {errors.login_id && <p className="mt-1 text-xs text-red-500">{errors.login_id}</p>}
                        </div>
