...
function TravelPageContent() {
    ...
    const mapDestinations = destinations.filter(
        (destination): destination is Destination & { position: [number, number] } => Boolean(destination.position),
    );
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-50">
            <div className="mx-auto max-w-[2000px] overflow-hidden bg-gradient-to-b from-slate-950 via-blue-950 to-slate-100">
                ...
                <section className="relative -mt-8 rounded-t-[2rem] bg-slate-100 px-4 pb-4 pt-10 text-slate-950 shadow-[0_-24px_80px_rgba(15,23,42,0.4)] sm:px-8">
                    <div className="mx-auto max-w-7xl">
                        <div className="mb-6 flex flex-col gap-2">
                            <span className="text-xs font-bold uppercase tracking-[0.35em] text-sky-700">Explore {currentMunicipality.name}</span>
                        ...
                        {!isLoading && !error && destinations.length === 0 && (
                            <div className="rounded-2xl border border-sky-100 bg-white p-8 text-center shadow-sm">
                                ...
                                <p className="mt-2 text-sm text-slate-600">Please check back soon for destinations in {currentMunicipality.name}.</p>
                            </div>
                        )}
