                        ...
                        dashboardList.map((item) => (
                            ...
                            >
                                ...
                                <TableCell className="text-[12px]">
                                    {item.dateApproved === '' ? 'Not yet approved' : Utility().formatToReadableDate(item.dateApproved)}
                                </TableCell>
                                <TableCell className="text-[12px]">{item.service}</TableCell>
                                <TableCell className="text-[12px]">{item.transactionNumber}</TableCell>
                                <TableCell className="text-[12px]">
                                    {item.dateApproved === '' ? 'Not yet approved' : Utility().formatAndAddDays(item.dateApproved, 160)}
                                </TableCell>
                                <TableCell>
                                    <span
                                        className={`rounded-full p-1 text-[11px] font-medium ${
                                            Utility().calculateTotalDays(item.dateApproved) >= 92
                                                ? 'bg-green-100 text-green-700'
                                                : 'bg-red-100 text-red-700'
                                        }`}
                                    >
                                        {Utility().calculateTotalDays(item.dateApproved) >= 92 ? 'Eligible' : 'Not Eligible'}
                                    </span>
                                </TableCell>
                            </TableRow>
                        ))
