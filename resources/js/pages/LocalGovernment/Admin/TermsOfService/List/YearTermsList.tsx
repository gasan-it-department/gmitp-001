...
import { Link } from '@inertiajs/react'; // Added router here
...
interface PageProps {
    ...
    currentMunicipality: MunicipalityType;
    [key: string]: any;
}

export default function YearTermsList({ terms, currentMunicipality }: PageProps) {
    // 1. Calculate the data on the fly (No useState needed!)
    const processData = (data: Term[]) => {
        ...
        return sorted.map((term) => ({
            ...term,
            is_current: term.label.toLowerCase().includes('(current)'),
            officials_count: 0,
        }));
    };

    const processedTerms = processData(terms.data);
    ...
    // Fix sticky pointer-events for dialogs
    useEffect(() => {
    ...
    const handleEdit = (id: string) => {
        // FIXED: Search the processed array, not the raw props object
        const term = processedTerms.find((t) => t.id === id);
        if (term) {
            setTermToEdit(term);
    ...
    const confirmDelete = () => {
        if (termToDelete) {
            // FIXED: Use Inertia router to hit your backend delete endpoint
            // Adjust the route name below to match your actual Laravel delete route
            // router.delete(
            //     government.admin.terms.destroy.url({
            ...
            <div className="relative min-h-screen w-full bg-slate-50/30">
                <div className="relative w-full p-6 md:p-10">
                    ...
                    <div className="space-y-4">
                        {/* FIXED: Use processedTerms instead of terms */}
                        {processedTerms.length === 0 ? (
                            ...
                            /* Terms List */
                            /* FIXED: Map over processedTerms instead of terms */
                                ...
                                >
                                    ...
                                    <div className="w-full flex-1 p-4 sm:p-0">
                                        ...
                                        <div className="flex items-end justify-between">
                                            <div>
                                                <h3
                                                    className={`text-2xl leading-none font-black tracking-tight sm:text-3xl ${term.is_current ? 'text-slate-900' : 'text-slate-700'}`}
                                                >
                                    ...
                                    <div className="flex w-full items-center justify-between gap-2 border-t border-slate-100 px-4 pt-3 pb-4 sm:w-auto sm:justify-end sm:border-0 sm:px-0 sm:pt-0 sm:pb-0">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Link
                                                ...
                                                className={`hidden h-8 w-8 items-center justify-center rounded-full transition-transform group-hover:translate-x-1 sm:flex ${term.is_current ? 'bg-blue-50 text-blue-500' : 'text-slate-300 group-hover:text-orange-500'} `}
                                            >
                    ...
                    initialData={
                        termToEdit
                            ? {
                                  ...
                                  isCurrent: termToEdit.is_current,
                              }
                            : null
                    }
                />
